...
// High-demand skills that are almost always required
...
// Curated resources for core skills
...
export default function SkillGapAnalyzerPage() {
  ...
  const analyzeGaps = async () => {
    ...
    requiredSkillsMap.forEach((value, skill) => {
      ...
      if (!hasSkill) {
        // Smart priority: Community JD > Multiple companies > Industry demand
    ...
    // Sort: high first, then medium, then low, alphabetically within each
    gaps.sort((a, b) => {
  ...
  const getAgeBadge = (createdAt: string, ageDays: number) => {
    if (ageDays < 7) {
      return { emoji: '🟢', text: 'Fresh', color: 'text-green-600', bg: 'bg-green-50' };
    } else if (ageDays < 30) {
      return { emoji: '🟡', text: `${ageDays} days ago`, color: 'text-yellow-600', bg: 'bg-yellow-50' };
    } else if (ageDays < 90) {
      return { emoji: '🟠', text: `${Math.floor(ageDays / 30)} months ago`, color: 'text-orange-600', bg: 'bg-orange-50' };
    } else if (ageDays < 180) {
      return { emoji: '🔴', text: `${Math.floor(ageDays / 30)} months ago`, color: 'text-red-600', bg: 'bg-red-50' };
    } else {
      return { emoji: '⚫', text: 'Archived', color: 'text-gray-500', bg: 'bg-gray-100' };
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {!showAddCompany ? (
          <button
            ...
            className="w-full bg-white border-2 border-dashed border-gray-300 rounded-xl p-4 text-gray-500 hover:border-blue-400 hover:text-blue-500 transition mb-6 font-medium"
          >
            + Add Company
          </button>
        ) : (
                  ...
                  <div key={idx} className={`border rounded-lg p-4 ${priorityBadge.bg} hover:shadow-sm transition`}>
                    <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                      ...
                      <div className="text-xs text-gray-500">
                        Required by: {gap.companies.join(', ')}
                      </div>
                    </div>
        ...
        {selectedCompanies.length === 0 && (
          <div className="bg-gray-50 rounded-xl p-12 text-center border border-gray-200">
            ...
            <p className="text-gray-400">
              Click "Add Company" above to start analyzing skill gaps.
            </p>
          </div>
        )}
      </div>
